 ...
 * Info:
    Int64 implementiert grundlegende Arithmetik für 64bit Integer, basierend auf Maschinencode (Deswegen sind die Funktionssignaturen
    auch in Maschinencode-Manier).

 * Inhalt:
 *  void mkf64(int dest, int hi, int lo)
        Schreibt lo und hi an einen Ort (dest).
 ...
 *  void add64(int dest, int src)
        Addiert src auf dest: dest <- dest+src
 ...
 *  void sub64(int dest, int src)
        Subtrahiert src von dest: dest <- dest-src
        dest : Ein Pointer auf ein Int64-Objekt oder einfach 8Byte freier Speicher. Wird verändert.
        src  : Ein Pointer auf ein Int64-Objekt. Wird nicht verändert.

 *  void mul64(int dest, int src)
        Multipliziert dest mit src: dest <- dest*src
        dest : Ein Pointer auf ein Int64-Objekt oder einfach 8Byte freier Speicher. Wird verändert.
        src  : Ein Pointer auf ein Int64-Objekt. Wird nicht verändert.

 *  void div64(int dest, int src)
        Dividiert dest durch src: dest <- dest/src
        dest : Ein Pointer auf ein Int64-Objekt oder einfach 8Byte freier Speicher. Wird verändert.
        src  : Ein Pointer auf ein Int64-Objekt. Wird nicht verändert.

 * Desweiteren bietet Int64 den Konstruktor int64@ für Int64-Objekte an - wer möchte, kann damit arbeiten, mk64 erwartet aber einen Pointer und kein Handle.
 */
